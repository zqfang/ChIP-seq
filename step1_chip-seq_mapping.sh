...
log "bowtie2 mapping"
mkdir -p bowtie_out
find . -name "*R1.fastq.gz" | while read var ; 
...
samtools view -@ 12 -bhS -q 25 ./bowtie_out/${id%%.*}.sam | samtools sort -@ 12  -T ./bowtie_out/${id%%.*} > ./bowtie_out/${id%%.*}.highQuality.sorted.bam
...
samtools sort  -@ 12  ./bowtie_out/${id%%.*}.unique.bam > ./bowtie_out/${id%%.*}.unique.sorted.bam  ## prefix for the output   
...
do
bedtools genomecov -ibam $id -g ~/genome/hg19_ucsc_fasta/hg19.chrom.sizes -split -bg |  sort -k1,1 -k2,2n  > bowtie_out/${id%%.*}.highQuality.sorted.bedgraph
...
bedGraphToBigWig ./bowtie_out/${id%%.*}.highQuality.sorted.bedgraph ~/genome/hg19_ucsc_fasta/hg19.chrom.sizes  bowtie_out/${id%%.*}.highQuality.bw
done
